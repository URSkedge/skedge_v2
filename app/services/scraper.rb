...
class Scraper
  ...
  def create_or_update_course_by(course_fields, course, s)
    department = course_fields.delete(:department)
    department_short = department.short
    num = course_fields[:number]

    c = department.courses.find_or_initialize_by(course_fields)
    ...
    c.comments = DataFormatter::Course.comments(department_short, num, course['classinfo']) || c.comments
  ...
  def scrape_courses(courses, department)
    ...
    courses.each do |course|
      ...
      if s.section_type == Section::Type::Course
        c = create_or_update_course_by(course_fields, course, s)
        c.save!
        ...
        nonmain_sections[course_fields] ||= []
        nonmain_sections[course_fields] << s
        next
      end
    end
